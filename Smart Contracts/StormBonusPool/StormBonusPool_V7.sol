...
contract StormBonusPool is Ownable, Pausable, ReentrancyGuard {
    ...
    function unstake(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        if (pendingReward > 0) {
            if (rewardToken.balanceOf(address(this)) >= pendingReward) {
                ...
                totalPendingRewards -= pendingReward; // Update total pending rewards
            } else {
    ...
    function autoCompound() external nonReentrant whenNotPaused {
        ...
        if (pendingReward > 0) {
            require(rewardToken.balanceOf(address(this)) >= pendingReward, "Insufficient reward token balance");
            bool rewardTransferSuccess = rewardToken.transfer(address(this), pendingReward);
            require(rewardTransferSuccess, "Reward transfer failed");
            totalRewardTokenBalance -= pendingReward; // Update total reward token balance
            totalPendingRewards -= pendingReward; // Update total pending rewards
            staker.stakedAmount += pendingReward; // Compound the pending rewards to staked amount
            staker.lastStakedTime = block.timestamp; // Reset lastStakedTime on each compound
            staker.lastPauseTime = lastPauseTime(); // Update last pause time
    ...
    function claimRewards() external nonReentrant whenNotPaused {
        ...
        uint256 pendingReward = staker.rewardDebt + staker.unpaidRewards;
        staker.rewardDebt = 0;
        staker.unpaidRewards = 0;
    ...
    function _updateReward(address _staker) internal {
        ...
        if (staker.stakedAmount > 0) {
            ...
            staker.lastStakedTime = block.timestamp; // Update last staked time
        }
    }

    function _checkRewardTokenBalance() internal {
        uint256 balance = rewardToken.balanceOf(address(this));
        if (balance < totalPendingRewards) {
            stakingPaused = true; // Pause staking if there are insufficient reward tokens
        } else {
            stakingPaused = false; // Unpause staking if there are sufficient reward tokens
        }
    }
    ...
    event InsufficientRewardTokens(address indexed user, uint256 amount);
}
